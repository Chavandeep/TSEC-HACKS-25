...
function Api() {
  ...
  const getDateRange = () => {
    ...
    switch (filter) {
      case 'daily':
        start.setDate(today.getDate() - 20); // 20 days ago
        break;
      case 'weekly':
        start.setDate(today.getDate() - (20 * 7)); // 20 weeks ago
        break;
      case 'monthly':
        start.setMonth(today.getMonth() - 20); // 20 months ago
        break;
      case 'past_month':
  ...
  const handleFetchData = async () => {
    ...
    if (!startDate || !endDate) {
      const { startDate: defaultStartDate, endDate: defaultEndDate } = getDateRange();
